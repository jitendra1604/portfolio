...
const nextConfig: NextConfig = {
  ...
  async headers() {
    return [{
      source: "/(.*)",
      headers: [
        { key: "Content-Security-Policy", value: "default-src 'self'; script-src 'self' 'unsafe-inline' https://va.vercel-scripts.com; style-src 'self' 'unsafe-inline'; img-src 'self' data: blob: https://avatars.githubusercontent.com https://images.unsplash.com https://plus.unsplash.com https://source.unsplash.com https://*.notion.so https://*.notion-static.com https://file.notion.so https://prod-files-secure.s3.us-west-2.amazonaws.com https://prod-files-secure.s3.us-east-2.amazonaws.com; font-src 'self' data:; connect-src 'self' https://api.github.com https://vitals.vercel-insights.com https://*.ably.io wss://*.ably.io https://*.ably-realtime.com wss://*.ably-realtime.com https://*.ably.net wss://*.ably.net; frame-ancestors 'none'; base-uri 'self'; form-action 'self'; object-src 'none'" },
